...
impl ScrollScreenshotService {
    ...
    fn compute_descriptor(&self, img: &image::GrayImage, corner: &ScrollOffset) -> Vec<f32> {
        ...
        for row in 0..(descriptor_size / 2) {
            ...
            descriptor.push(if valid_pixels > 0 { sum / valid_pixels as f32 } else { 0.0 });
        }
    ...
    fn get_descriptors(
        ...
        corners.par_iter().map(|corner| self.compute_descriptor(image, corner)).collect()
    }
    ...
    fn get_gray_image(&mut self, image: &DynamicImage) -> GrayImage {
        ...
        let src_image =
            Image::from_slice_u8(image_width, image_height, gray_image.as_mut(), PixelType::U8)
                .unwrap();
        ...
        GrayImage::from_vec(self.image_dst_width, self.image_dst_height, dst_image.into_vec())
            .unwrap()
    }
    ...
    fn get_crop_region(&self, delta_size: i32) -> CropRegion {
        ...
        if self.current_direction == ScrollDirection::Vertical {
            ...
            if delta_size > 0 {
                region =
                    CropRegion::new(0, start_position, image_width, image_height - start_position);
            } else {
            ...
            if start_position > 0 {
                region =
                    CropRegion::new(start_position, 0, image_width - start_position, image_height);
            } else {
    ...
    fn get_corners(&mut self, image: &image::GrayImage) -> Vec<ScrollOffset> {
        ...
        corners
            .iter()
            .map(|corner| ScrollOffset { x: corner.x as i32, y: corner.y as i32 })
            .collect()
    }
    ...
    fn build_index(
        ...
        new_scroll_index.descriptors.iter().enumerate().for_each(|(i, descriptor)| {
    ...
    fn add_index(
        ...
        let image_overlay_size =
            if delta_size > 0 { image_overlay_size } else { -image_overlay_size };
    ...
    pub fn get_offsets<'a>(
        ...
        let offsets: Vec<(i32, &'a ScrollIndex, usize, usize)> = image_descriptors
            ...
            .filter_map(|(i, descriptor)| {
                ...
                if dist < 0.1 { Some((diff, index, idx1, i)) } else { None }
            })
            .collect();
        ...
        for (offset, scroll_index, origin_position_index, new_position_index) in offsets {
            if let Some(value) = offset_counts.get_mut(&offset) {
                ...
                offset_counts
                    .insert(offset, (1, scroll_index, origin_position_index, new_position_index));
            }
        }
    ...
    pub fn handle_image(
        ...
        image: DynamicImage,
        scroll_image_list: ScrollImageList,
    ) -> (Option<(i32, Option<ScrollImageList>)>, bool, ScrollImageList) {
        let image_width = image.width();
        let image_height = image.height();
        ...
        if self.top_image_list.is_empty() && self.bottom_image_list.is_empty() {
            ...
            new_top_image_ann_index.descriptors.iter().enumerate().for_each(|(i, descriptor)| {
                new_top_image_ann_index.ann_index.add(descriptor, i).unwrap();
            });

            new_top_image_ann_index.ann_index.build(Metric::Euclidean).unwrap();
        ...
        // 从边缘遍历
        let mut offsets;
        let (first_offsets, is_origin) =
            self.get_offsets(first_index, &image_descriptors, &image_corners, scroll_image_list);
    ...
    pub fn export(&mut self) -> Option<image::DynamicImage> {
        ...
        let (total_width, total_height) = if self.current_direction == ScrollDirection::Vertical {
            (self.image_width as usize, (self.top_image_size + self.bottom_image_size) as usize)
        } else {
            ((self.top_image_size + self.bottom_image_size) as usize, self.image_height as usize)
        };
        ...
        for scroll_image in self.bottom_image_list.iter() {
            ...
            if self.current_direction == ScrollDirection::Vertical {
                // 垂直拼接
                crate::adapters::image::overlay_image(
                    &mut final_image,
                    total_width,
                    img,
                    0,
